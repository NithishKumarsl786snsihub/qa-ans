...
export default function Home() {
  ...
  const guardedTabChange = (tab: AppTab) => {
    if (testState && !testState.completedAt && tab !== "test") {
      const ok = window.confirm("A test is in progress. Leave Test Mode and keep progress in the background?");
      if (!ok) return;
    }
    setActiveTab(tab);
  };
...
function TestModeView({
  ...
  const finishTest = () => {
    const ok = window.confirm("Submit the test now? Unanswered questions will be counted in the result.");
    if (!ok) return;
